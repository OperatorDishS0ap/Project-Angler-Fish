...
ADC_ADDRESS_ENV = os.environ.get("ANGLERFISH_ADS1015_ADDR")
...
def _candidate_ads_addresses():
    if ADC_ADDRESS_ENV:
        try:
            return [int(ADC_ADDRESS_ENV, 0)]
        except ValueError:
            print(f"[ads1015] Invalid ANGLERFISH_ADS1015_ADDR={ADC_ADDRESS_ENV!r}; using auto scan")

    # ADS1015 supports 0x48-0x4B depending on ADDR pin wiring.
    return [0x48, 0x49, 0x4A, 0x4B]


def _scan_i2c_addresses(i2c, timeout_s: float = 1.0):
...
def init_ads1015():
    ...
    try:
        ...
        candidates = _candidate_ads_addresses()
        detected = _scan_i2c_addresses(i2c)
        if detected:
            detected_hex = ", ".join(f"0x{addr:02x}" for addr in detected)
            print(f"[ads1015] I2C devices detected: {detected_hex}")
        ...
        for addr in candidates:
            try:
                ...
                print(f"[ads1015] Initialized at 0x{addr:02x} gain={ADC_GAIN}")
                return channels
            except Exception as exc:
                print(f"[ads1015] Probe failed at 0x{addr:02x}: {exc}")

        candidates_hex = ", ".join(f"0x{addr:02x}" for addr in candidates)
        print(f"[ads1015] Init failed; no ADS1015 at [{candidates_hex}]")
        return None
    except Exception as exc:
